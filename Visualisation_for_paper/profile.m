...
classdef profile
    ...
    methods
        function obj = profile(profile_vertex_coords, inset)
            ...
            obj.bounding_box = [100, 100; -100, -100];

            for i = 1:size(obj.vertex_coords, 1);
                    
                    obj.bounding_box(1, 1) = min(obj.vertex_coords(i, 1), obj.bounding_box(1, 1));
                    obj.bounding_box(1, 2) = min(obj.vertex_coords(i, 2), obj.bounding_box(1, 2));
                    obj.bounding_box(2, 1) = max(obj.vertex_coords(i, 1), obj.bounding_box(2, 1));
                    obj.bounding_box(2, 2) = max(obj.vertex_coords(i, 2), obj.bounding_box(2, 2));
        ...
        function obj = scale_planar(obj, scale_factor)
            ...
            temp_bounds = obj.bounding_box - repmat(obj.centroid, [2,1]);
            temp_bounds = temp_bounds*scale_factor;
            obj.bounding_box = temp_bounds + repmat(obj.centroid, [2,1]);
