...
classdef extrusion
    ...
    properties
        ...
        % 2 x 3 matrix of min and max points of bounding box
        bounding_box;
        % 2 x 3 matrix of bounds following transformations
    ...
    methods
        ...
        function obj = extrusion(base_profile, extrude_marks, xy_pos_vals, scale_vals, density, additionalMass, massPos)
            ...
            %Gets bounding box
            obj.bounding_box = [100, 100, 100; -100, -100, -100];

            %Z values
            obj.bounding_box(1, 3) = min(extrude_marks);
            obj.bounding_box(2, 3) = max(extrude_marks);
            ...
            for profile_index = 1:num_profiles
                ...
                obj.bounding_box(1, 1) = min(obj.profiles{profile_index}.bounding_box(1, 1), ...
                    obj.bounding_box(1, 1));
                obj.bounding_box(1, 2) = min(obj.profiles{profile_index}.bounding_box(1, 2), ...
                    obj.bounding_box(1, 2));
                obj.bounding_box(2, 1) = max(obj.profiles{profile_index}.bounding_box(2, 1), ...
                    obj.bounding_box(2, 1));
                obj.bounding_box(2, 2) = max(obj.profiles{profile_index}.bounding_box(2, 2), ...
                    obj.bounding_box(2, 2));

            end

            
            boundSize = obj.bounding_box(2, :) - obj.bounding_box(1, :);
            obj.bounding_box(1, :) = obj.bounding_box(1, :) - 0.05*boundSize;
            obj.bounding_box(2, :) = obj.bounding_box(2, :) + 0.05*boundSize;
        ...
        function obj = translate_part(obj, translation)
            ...
            obj.bounding_box = obj.bounding_box + repmat(translation, [2, 1]);
        ...
        function obj = rotate_part(obj, rotation_quat, pivot_point)
            ...
            temp_bounding_box = obj.bounding_box - repmat(pivot_point, [2, 1]);
            ...
            temp_bounding_box = quatrotate(rotation_quaternion, temp_bounding_box);
            ...
            temp_bounding_box= temp_bounding_box + repmat(pivot_point, [2, 1]);

            %Reorganises bounding box
            obj.bounding_box(1, :) = min(temp_bounding_box, [], 1);
            obj.bounding_box(2, :) = max(temp_bounding_box, [], 1);
