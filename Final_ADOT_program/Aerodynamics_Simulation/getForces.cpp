...
pair<glm::dvec3, glm::dvec3> getForces(string filePath, string latestTime){
    ...
    //Gets matched values
    pressureForce[0] = stod(forceInfo.str(2));
    pressureForce[1] = stod(forceInfo.str(3));
    pressureForce[2] = stod(forceInfo.str(4));

    viscousForce[0] = stod(forceInfo.str(5));
    viscousForce[1] = stod(forceInfo.str(6));
    viscousForce[2] = stod(forceInfo.str(7));

    pressureTorque[0] = stod(forceInfo.str(8));
    pressureTorque[1] = stod(forceInfo.str(9));
    pressureTorque[2] = stod(forceInfo.str(10));

    viscousTorque[0] = stod(forceInfo.str(11));
...
pair<glm::dvec3, glm::dvec3> calculateForces(string filePath, glm::dvec3 COM, const double g, const double rho){    
    ...
    //Opens field files
    string finalTime = "0.6";

    ifstream pressureFile;
    pressureFile.open(filePath + finalTime + "/p");
    ifstream velocityFile;
    velocityFile.open(filePath + finalTime + "/U");
