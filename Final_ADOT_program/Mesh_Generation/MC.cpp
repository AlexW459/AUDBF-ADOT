...
namespace MC
{
	...
	*/
        ...
        double vb, int axis, muint x, muint y, muint z, const glm::ivec3& size)
	{
		...
		glm::dvec3 v(x, y, z);
		v[axis] += va / (va - vb);
		slab_inds[mc_internalToIndex1DSlab(x, y, z, size)][axis] = (int)mesh.vertices.size();
		mesh.vertices.push_back(v);
		mesh.normals.push_back(glm::dvec3(0.0));
	}
	...
	*/
	void mc_internalAccumulateNormal(mcMesh& mesh, muint a, muint b, muint c)
	{
		glm::dvec3& va = mesh.vertices[a];
		glm::dvec3& vb = mesh.vertices[b];
		glm::dvec3& vc = mesh.vertices[c];
		glm::dvec3 ab = va - vb;
	    glm::dvec3 cb = vc - vb;
		glm::dvec3 n = glm::cross(cb, ab);
		mesh.normals[a] += n;
		mesh.normals[b] += n;
		mesh.normals[c] += n;
	}
	...
	*/
	void marching_cube(const std::vector<double>& field, muint nx, muint ny, muint nz, mcMesh& outputMesh)
	{
		...
		const glm::ivec3 size(nx, ny, nz);
		std::vector<glm::ivec3> slab_inds(nx * ny * 2);
		std::array<double, 8> vs = { 0, 0, 0, 0, 0, 0, 0, 0 };
		std::array<muint, 12> edge_indices;
		...
		for (size_t i = 0; i < outputMesh.normals.size(); i++)
			outputMesh.normals[i] = glm::normalize(outputMesh.normals[i]);;
	}
