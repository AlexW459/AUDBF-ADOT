...
// glfw: initialize and configure
    ...
    FILE *my_file;
    my_file = fopen("../nodes2coords.txt", "r");
    unsigned int i = 0 ;
    
    while (!feof(my_file))
    { 
        fscanf(my_file, "  %f", &nodes2coords[i]);
        //printf("%f\n",nodes2coords[i] );
        i++;
    }
    ...
    FILE *my_file_facets;
    my_file_facets = fopen("../facets2nodes.txt", "r");
    i = 0 ;
    ...
    */
    ...
    glBufferData(GL_ARRAY_BUFFER, 3*num_nodes*sizeof(float), nodes2coords, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3*num_facets*sizeof(unsigned int), facets2nodes, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        glDrawElements(GL_TRIANGLES, 3*num_facets, GL_UNSIGNED_INT, 0);
