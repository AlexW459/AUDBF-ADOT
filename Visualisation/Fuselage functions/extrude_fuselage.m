...
function [fuselage_extrusion, pivot_point] = extrude_fuselage(profiles, ...
    ...
    nose_scale_vals = (nose_bez_profile(:, 1) - nose_bez_profile(:, 2))./fuselage_height;
    nose_y_pos_vals = (nose_bez_profile(:, 1) + nose_bez_profile(:, 2))./2;
    ...
    tail_scale_vals = (tail_bez_profile(:, 1) - tail_bez_profile(:, 2))./fuselage_height;
    tail_y_pos_vals = (tail_bez_profile(:, 1) + tail_bez_profile(:, 2))./2;
    ...
    z_sample = [tail_x_sample(end-1:-1:1), 0, fuselage_length, nose_x_sample(1:end-1)+fuselage_length];

    %Finds the side profile of the extrusion
    y_pos_vals = [tail_y_pos_vals(end-1:-1:1); 0; 0; nose_y_pos_vals(1:end-1)];
    x_pos_vals = zeros(size(z_sample, 2), 1);

    scale_vals = [tail_scale_vals(end-1:-1:1); 1; 1; nose_scale_vals(1:end-1)]';

    xy_pos_vals = [x_pos_vals, y_pos_vals];
    

    %plot(x_sample', side_profile_matrix(:, 1), x_sample', side_profile_matrix(:, 2));
   
    fuselage_extrusion = extrusion(fuselage_base_profile, z_sample, ...
        xy_pos_vals, scale_vals, density, battery_mass, battery_pos*[0, 0, fuselage_length]);
